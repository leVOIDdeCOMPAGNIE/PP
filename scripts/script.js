...
/*Изменение цвета региона по наведению на пин и регион*/
pinOne.addEventListener('mouseover', () => {
  region.style.fill = '#6d097a';
});
pinOne.addEventListener('mouseout', () => {
  region.style.fill = '#9a34a8';
});
pinTwo.addEventListener('mouseover', () => {
  region.style.fill = '#6d097a';
});
pinTwo.addEventListener('mouseout', () => {
  region.style.fill = '#9a34a8';
});
region.onmouseout = function (event) {
  let target = event.target;
  target.style.fill = '#9a34a8';
};
region.onmouseover = function (event) {
  let target = event.target;
  target.style.fill = '#6d097a';
};
